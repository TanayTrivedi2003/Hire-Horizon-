...
const ROLES = ['Branch Manager', 'Assistant Manager', 'Deputy Manager', 'Relationship Manager', 'Operations Executive', 'Customer Service Executive'];

export default function ApplicationForm() {
  const [sent, setSent] = useState(false);
  const [form, setForm] = useState({ name: '', mobile: '', email: '', qualification: '', role: '', city: '' });
  ...
  const validate = () => {
    ...
    else if (!/^\+?[\d\s-]{10,15}$/.test(form.mobile)) e.mobile = 'Enter a valid number';
  ...
  const submit = (ev: React.FormEvent) => {
    ...
    setTimeout(() => {
      setSent(false);
      setForm({ name: '', mobile: '', email: '', qualification: '', role: '', city: '' });
    }, 3500);
  };

  const field = (name: keyof typeof form, label: string, type = 'text', select = false) => (
    <div className="relative">
      {select ? (
        ...
        >
          ...
          {ROLES.map((r) => <option key={r} value={r} className="bg-navy-950">{r}</option>)}
        </select>
      ) : (
        <input
          id={name}
  ...
  return (
    <section id="apply" className="relative py-24 md:py-32 bg-navy-950 overflow-hidden">
        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-5">
            <SectionTitle
              ...
              title={<>One short form.<br />A <span className="text-gold-gradient italic">career-defining call.</span></>}
          ...
          <div className="lg:col-span-7">
            ...
            >
              ...
              <button type="submit" disabled={sent} className="btn-primary w-full mt-2 disabled:opacity-80">
                {sent ? (<><Check className="w-4 h-4" /> Application Received</>) : (<>Start Your Banking Career Today <Send className="w-4 h-4" /></>)}
              </button>
              <p className="text-xs text-white/40 text-center">
                By submitting you agree to be contacted by Hire Horizon Group. Your data stays confidential.
              </p>
            </motion.form>
          </div>
        </div>
